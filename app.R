...
completeList <- read.csv("Canada_species_collapsibleTree_by_mean_obs.csv", sep=";")
completeList[completeList==0] <-NA
completeList$Observations[is.na(completeList$Observations)] <- 0
completeList$observations_per_species[is.na(completeList$observations_per_species)] <- 0
...
## Instructions for the computer to run the app
server <- function(input, output, session){
  ...
  output$pie <- renderPlot({
    ...
    ################ section for capturing the level you clicked ###############
    ...
    subtitle <- paste(
      ...
      paste(format(nrow(df), big.mark = ","), "species"),
      # showing total number of observations
      paste(format(sum(df$observations_per_species), big.mark = ","), "observations"),
  ...
  output$download_filtered_df <- downloadHandler(
    ...
    content = function(file) {
      ...
      df$observations[is.na(df$observations_per_species)] <- 0
